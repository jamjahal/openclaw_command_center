...
import { useMockProvider } from "./providers/useMockProvider";
import { useLiveProvider } from "./providers/useLiveProvider";
import { ROOM_W, ROOM_H, SYNC_CENTER } from "./providers/types";

// ─── Icon Map ───────────────────────────────────────────
const IconMap = { Calendar, BarChart3, Code, Headphones, BookOpen, Network, Monitor, Database, Cpu, Server, Workflow, Radio };

// ─── Configuration ──────────────────────────────────────
// Switch between providers here. To connect to a live OpenClaw backend,
// set MODE to "live" and point LIVE_WS_URL at your WebSocket endpoint.
const MODE = import.meta.env.VITE_PROVIDER_MODE || "demo";
const LIVE_WS_URL = import.meta.env.VITE_WS_URL || "ws://localhost:8080/agents";

// ─── Provider Hook ──────────────────────────────────────
// Thin wrapper that selects the right provider based on MODE.
// Both return the exact same shape, so the rest of the component
// doesn't know or care which one is active.
function useAgentData() {
  const mock = useMockProvider();
  const live = useLiveProvider({ url: MODE === "live" ? LIVE_WS_URL : null });
...
export default function AgentRoom3D() {
  // ── Data from provider ──────────────────────────────
  const { agents, collabPairs, activityLog, workstations, mode, connected } = useAgentData();
      ...
      <div className="relative z-10 flex items-center justify-between px-4 py-3" style={{
        ...
        <div className="flex gap-3 items-center" data-no-drag>
          ...
          <div style={{
            ...
            background: "rgba(0,0,0,0.8)", borderRadius: 6, padding: "4px 10px",
            border: `1px solid ${mode === "live" ? (connected ? "rgba(34,197,94,0.5)" : "rgba(239,68,68,0.5)") : "rgba(168,85,247,0.5)"}`,
          }}>
            {mode === "live"
              ? (connected
                ? <Wifi size={12} color="#22c55e" />
                : <WifiOff size={12} color="#ef4444" />)
              : <span style={{ fontSize: 10, color: "#a855f7" }}>DEMO</span>}
            <span style={{ fontSize: 10, color: mode === "live" ? (connected ? "#22c55e" : "#ef4444") : "#a855f7" }}>
              {mode === "live" ? (connected ? "LIVE" : "RECONNECTING") : "MOCK"}
            </span>
          </div>
